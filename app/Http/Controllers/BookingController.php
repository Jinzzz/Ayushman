...
class BookingController extends Controller
{
    ...
    //   if ($request->filled('payable_amount') && is_array($request->input('payable_amount'))) {

    //         dd($request->input('payable_amount'));
    //     }else{
    //         dd("no");
    //     }
        $request->validate([
    ...
    public function TherapyRefundStore(Request $request)
    {
       ...
       $therapy_refund->save();
        ...
        Trn_Consultation_Booking::where('id',$request->booking_id)->update(['booking_status_id'=>90]);
