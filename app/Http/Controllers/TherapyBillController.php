...
class TherapyBillController extends Controller
{
        public function therapyBill(Request $request)
    {
    ...
    return view('therapy-bill.index', compact('patients', 'pageTitle','patientLists'));
    }
    else{
        $patients = Trn_Consultation_Booking::leftJoin('mst_patients', 'mst_patients.id', '=', 'trn_consultation_bookings.patient_id')
        ->leftJoin('mst_master_values as booking_type', 'booking_type.id', '=', 'trn_consultation_bookings.booking_type_id')
    ...
    public function generateInvoice($id)
    {
        ...
        return view('therapy-bill.create', compact('invoice', 'pageTitle','patientLists','paymentType','discount','booking_id'));
    }
    ...
    public function saveInvoice(Request $request, $id)
    {
        ...
        foreach ($request->input('amount') as $key => $value) {
            ...
            $invoicePayment->therapy_booking_invoice_id = $invoice->id;
        ...
        if ($request->has('payment_mode') && $request->has('deposit_to')) {
            ...
            $invoice->amount = $request->input('total-amount');
            $invoice->is_paid = 1;
            $invoice->save();
                        ...
                        Trn_Ledger_Posting::create([
                            ...
                            'debit' =>  $request->input('total-amount'),
                            'credit' => 0,
                        ...
                        Trn_Ledger_Posting::create([
                            ...
                            'entity_id' => 0,
                            'debit' => 0,
                            'credit' =>  $request->input('total-amount'),
                            'branch_id' => $request->input('branch_id'),
                            'transaction_id' =>  $booking->id,
                            'narration' => 'Wellness Booking Invoice Payment'
                        ]);
        ...
        return redirect()->route('therapy-bill.index')->with('success', 'Invoice created successfully');
    }
}