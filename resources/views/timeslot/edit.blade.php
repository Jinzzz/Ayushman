                ...
                <div class="table-responsive">
                    <table id="example" class="table table-striped table-bordered text-nowrap w-100">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ ++$i }}</td>
                                <td>{{ $time->master_value }}</td>
                                <td>
                                    <form action="{{ route('timeslot.changeStatus', $time->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            onclick="return confirm('Do you want to Change status?');"
                                            class="btn btn-sm @if($time->is_active == 0) btn-danger @else btn-success @endif">
                                            @if($time->is_active == 0)
                                            InActive
                                            @else
                                            Active
                                ...
                                <td>
                                    <a class="btn btn-primary btn-sm edit-custom"
                                        href="{{ route('timeslot.edit', $time->id) }}"><i
                                            class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit </a>
            
                                    <form style="display: inline-block"
                                        action="{{ route('timeslot.destroy', $time->id) }}" method="post">
                                        @csrf
                                        @method('delete')
